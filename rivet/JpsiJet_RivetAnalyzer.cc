...
public:
  /// Constructor
  DEFAULT_RIVET_ANALYSIS_CTOR(JpsiJet_RivetAnalyzer);
  ...
  void init() {
    ...
    //      FastJets jet4(fs, FastJets::ANTIKT, 0.4); // R = 0.4 jets
    FastJets jet4(fs, FastJets::ANTIKT, 0.4, JetAlg::Muons::NONE, JetAlg::Invisibles::NONE); // R = 0.8 jets
    declare(jet4, "jet4");

    // Particles for the jets
    VetoedFinalState jet_input(fs);
    const UnstableParticles jpsi_fs =
        UnstableParticles(Cuts::abspid == PID::JPSI && Cuts::abseta < 2.4 &&
    //FinalState jpsi_fs =
    ...
    book(_hZ, "zJpsi", {0.16, 0.22, 0.298, 0.376, 0.454, 0.532, 0.610,  0.688, 0.766, 0.844, 0.922, 1.0});
    book(_hZqqcc, "zJpsiFromqqbar", {0.16, 0.22, 0.298, 0.376, 0.454, 0.532, 0.610,  0.688, 0.766, 0.844, 0.922, 1.0});
    book(_hZgcc, "zJpsiFromgtocc", {0.16, 0.22, 0.298, 0.376, 0.454, 0.532, 0.610,  0.688, 0.766, 0.844, 0.922, 1.0});
  ...
  void analyze(const Event &event) {
    ...
    printf("Start event\n" );
#endif
    //const UnstableParticles& unstable = apply<UnstableParticles>(event, "UFS");
...
//        apply<FinalState>(event, "Jpsi").particles();
    ...
    const Particles& jpsi_particles = apply<UnstableParticles>(event, "Jpsi").particles();
    if (jpsi_particles.size() == 0) vetoEvent;
    bool isNonPrompt = jpsi_particles[0].fromBottom();
    if( isNonPrompt ) vetoEvent;

//    Jets jets = apply<FastJets>(event, "jet4").jetsByPt(Cuts::pT > 30 * GeV).jetsByPt(Cuts::pT < 40 * GeV);
...
//      if(fsParticles[iFS].abspid() == 13 )printf("pseudo jet : %d, %f %f\n",fsParticles[iFS].pid(), fsParticles[iFS].eta(), fsParticles[iFS].phi() );
      for( auto dau : toCheck){
        if(fabs((fsParticles[iFS].eta() - dau.eta()) <1e-4) && fabs(fsParticles[iFS].phi() - dau.phi()) < 1e-4) p.set_user_index(1313); // FastJet index used to keep track of J/psi
//       if(fsParticles[iFS].abspid() == 13 ) printf("Check : %d\n", p.user_index());
      }
      particles.push_back(p);
    }

    JetDefinition jetDefAKT_Sig(fastjet::antikt_algorithm, 0.4); // jet distance parameter
    ClusterSequence antikTjets(particles, jetDefAKT_Sig); // cluster anti-kT
    vector<PseudoJet> jets = fastjet::sorted_by_pt(antikTjets.inclusive_jets(30)); // Jet projection : we only keep jets with pT > 30 Gev

  //  printf("Pseudo jet size : %d, Clustered jet size : %d\n", (int) particles.size(), (int) jets.size());
   ...
   _hJetpT->fill(jets[0].pt());
    ...
    for (size_t i = 0; i < jets.size(); ++i) // loop over all jets
    {
      ...
      for (const auto& constituent : constituents) {
        if(constituent.user_index() == 1313)hasJpsi += 1;
      }
#ifdef JpsiDEBUG
      ...
      printf("z value: %f\n", z);
      assert(z > 0);
      if(isFromGtoCC(jpsi_particles[0])) _hZgcc->fill(z);
      else _hZqqcc->fill(z);
...
//    		printf(" : child PID : %d\n", child.pid());
        ...
        // Add the parents of the current particle to the list for further checking
...
DECLARE_RIVET_PLUGIN(JpsiJet_RivetAnalyzer);
